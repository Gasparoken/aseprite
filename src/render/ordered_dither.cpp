  ...
  const doc::Palette* palette)
{
  ...
  if (a == 0)
    nearest1idx = m_transparentIndex;
  else if (octreeMap)
    nearest1idx = octreeMap->mapColor(r, g, b);
  else if (rgbmap)
  ...
  if (a2 == 0)
    nearest2idx = m_transparentIndex;
  else if (octreeMap)
    nearest2idx = octreeMap->mapColor(r2, g2, b2);
  else if (rgbmap)
  ...
  const doc::Palette* palette)
{
  ...
  int index;
  if (a == 0)
    index = m_transparentIndex;
  else if (octreeMap)
    index = octreeMap->mapColor(r, g, b);
  else if (rgbmap)

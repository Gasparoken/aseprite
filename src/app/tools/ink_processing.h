...
public:
  ...
  RgbTraits::pixel_t shadindProcess(int i, RgbTraits::pixel_t src) {
  ...
  void processPixel(int x, int y) {
    ...
    *m_dstAddress = shadindProcess(i, src);
  }
...
public:
  ...
  typename GrayscaleTraits::pixel_t shadindProcess(int i, typename GrayscaleTraits::pixel_t src) {
    if (i < 0) {
      return src;
    }
...
public:
  ...
  typename IndexedTraits::pixel_t shadindProcess(int i, typename IndexedTraits::pixel_t src) {
    if (m_remap) {
      i = (*m_remap)[i];
    }
    else {
...
RgbTraits::pixel_t BrushShadingInkProcessing<RgbTraits>::shadingProcessPixel(int iSrc, int iBrush, color_t initialSrc) {
  ...
  if (iSrc < 0 ||  iBrush < 0)
    return initialSrc;
  return m_shading.shadindProcess(iSrc, initialSrc);
}
